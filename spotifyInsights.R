  ...
  dashboardBody(
    ...
    tabItems(
      tabItem(tabName = "description", 
              ...
              h3("Procedure for Getting Your Own Personal Insights"),
              h4("NOTE: Make sure that your Spotify email is registered to use the application."),
...
getAccessToken <- function(code) {
  clientID <- "78f019feedaa4f4e9f35c9938b593b59"
  clientSecret <- "81878efc0d05402fa7cd95ca096d4db3"
...
outputDefault <- function(output, db_topArtists, db_topTracks) {
  # Output Top Artists
  db <- db_topArtists %>% 
    group_by(genre) %>% 
  ...
  output$plot02 <- renderPlot({
    ...
    mtext("Artist", side = 1, line = 9, font = 2, at = 2.5)
  })
...
server <- function(input, output, session) {
  ...
  # Gets the default data by refreshing the access token and outputting the default tables and plots
  refresh_token = "AQD5fxIsZDi5U_H7hAPaMwg7yKkcrKCCVYT5DI5BTgSgGnOmHCK01wXy1sncmFwEu09eHSdQZIJ4X22rLYDv1WXJLsRosaq-6jKRJrxqWbzOZX6hIUEB0ZpxzuqsUjYwvwY"
  data = list(grant_type = "refresh_token", refresh_token = refresh_token, client_id = clientID, client_secret = clientSecret)
  encoded_data <- paste0(names(data), "=", gsub(" ", "+", URLencode(data)), collapse = "&")
  response <- POST(
    url = "https://accounts.spotify.com/api/token",
    add_headers("Content-Type" = "application/x-www-form-urlencoded"),
    body = encoded_data
  )
  access_token <- fromJSON(content(response, as = "text"))$access_token
  shared_data <- reactiveValues(access_token = access_token)
  ...
  # Output Default Plots and Tables
  outputDefault(output, db_topArtists, db_topTracks)
  ...
  # Observe the User Chooses Options for Plot02
